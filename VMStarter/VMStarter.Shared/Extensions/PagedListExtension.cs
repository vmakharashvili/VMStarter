...
namespace VMStarter.Shared.Extensions
{
    public static class PagedListExtension
    {
        ...
        public static async Task<PagedList<Aggregate>> ToPagedListAsync<Aggregate, FilterObj>(this IQueryable<Aggregate> list, Filter filter, Func<Aggregate, string> order = null) where FilterObj : Filter
        {
            PagedList<Aggregate> myList = new PagedList<Aggregate>();
            myList.TotalNumber = await list.CountAsync();
            myList.List = await list.OrderBy(f => order(f)).Skip((filter.Skip == null && filter.Take == null) ? 0 : (filter.Skip.Value * filter.Take.Value))
                .Take((filter.Skip == null && filter.Take == null) ? myList.TotalNumber : filter.Take.Value).ToListAsync();
            return myList;
        }
        ...
        public static async Task<PagedList<ViewTable>> ToMappedPagedListAsync<DBTable, ViewTable, FilterObj>(this IQueryable<DBTable> dblist,
        Filter filter, Func<DBTable, bool> func = null, Func<ViewTable, string> order = null)
        where FilterObj : Filter where DBTable : IDatabase
        {
            PagedList<ViewTable> list = new PagedList<ViewTable>();
            list.TotalNumber = await dblist.CountAsync();
            list.List = await dblist.Where(f => func == null || func(f)).Select(f => f.Map<ViewTable>()).OrderBy(f => order(f)).Skip((filter.Skip == null && filter.Take == null) ? 0 : (filter.Skip.Value * filter.Take.Value))
            .Take((filter.Skip == null && filter.Take == null) ? list.TotalNumber : filter.Take.Value).ToListAsync();
            return list;
        }
    }
}
